...
public class IndicatorSeekBar extends View {
    ...
    private void drawThumbText(Canvas canvas) {
        ...
        if (mSeekChangeListener != null) {
            String thumbText = mSeekChangeListener.onDrawThumbText(mProgress);
            if (TextUtils.isEmpty(thumbText)) {
                thumbText = getProgressString(mProgress);
            }
            float x = getThumbCenterX();
            int textWidth = thumbText.length() * 4;
            if (x - textWidth <= mPaddingLeft) {
                x += textWidth;
            } else if (x + textWidth + mPaddingRight >= getWidth()) {
                x -= textWidth;
            }
            canvas.drawText(thumbText, x, mThumbTextY, mTextPaint);
            return;
        }
